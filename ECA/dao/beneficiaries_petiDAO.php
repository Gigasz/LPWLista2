...
class beneficiaries_petiDAO
{
    ...
    public function remover($beneficiaries)
    {
        global $pdo;
        try {
            $statement = $pdo->prepare("DELETE FROM tb_beneficiaries_peti WHERE id_beneficiaries = :id");
    ...
    {//id_beneficiaries, int_ano_mes_ref, str_uf, int_cod_siaf,str_nome_municipio , str_nis_favorecido, str_sit_benefic, int_valor_parcela, str_nome_favorecido
        global $pdo;
        try {
            if ($beneficiaries->getIdBeneficiaries() != "") {
                $statement = $pdo->prepare("UPDATE tb_beneficiaries_peti SET int_ano_mes_ref=:int_ano_mes_ref, str_uf=:str_uf, int_cod_siaf=:int_cod_siaf, str_nome_municipio=:str_nome_municipio, str_nis_favorecido=:str_nis_favorecido, str_sit_benefic=:str_sit_benefic, int_valor_parcela=:int_valor_parcela, str_nome_favorecido=:str_nome_favorecido WHERE id_beneficiaries = :id;");
                $statement->bindValue(":id", $beneficiaries->getIdBeneficiaries());
            } else {
                $statement = $pdo->prepare("INSERT INTO tb_beneficiaries_peti (id_beneficiaries, int_ano_mes_ref, str_uf, int_cod_siaf, str_nome_municipio, str_nis_favorecido, str_sit_benefic, int_valor_parcela, str_nome_favorecido) VALUES (:int_ano_mes_ref, :str_uf, :int_cod_siaf, :str_nome_municipio, :str_nis_favorecido, :str_sit_benefic, :int_valor_parcela, :str_nome_favorecido)");
            }

            $statement->bindValue(":int_ano_mes_ref", $beneficiaries->getIntAnoMesRef());
            $statement->bindValue(":str_uf", $beneficiaries->getStrUf());
            $statement->bindValue(":int_cod_siaf", $beneficiaries->getIntCodSiaf());
            ...
            $statement->bindValue(":str_sit_benefic", $beneficiaries->getStrSitBenefic());
    ...
    public function atualizar($beneficiaries)
    {
        global $pdo;
        try {
            $statement = $pdo->prepare("SELECT id_beneficiaries, int_ano_mes_ref, str_uf, int_cod_siaf, str_nome_municipio, str_nis_favorecido, str_sit_benefic, int_valor_parcela, str_nome_favorecido FROM tb_beneficiaries_peti WHERE id_beneficiaries = :id");
            $statement->bindValue(":id", $beneficiaries->getIdBeneficiaries());
            if ($statement->execute()) {
                ...
                $beneficiaries->setIntCodSiaf($rs->str_uf);
                $beneficiaries->setStrNomeMunicipio($rs->int_cod_siaf);
                $beneficiaries->setStrNomeMunicipio($rs->str_nome_municipio);
                $beneficiaries->setStrNisFavorecido($rs->str_nis_favorecido);
                $beneficiaries->setStrSitBenefic($rs->str_sit_benefic);
    ...
    public function tabelapaginada()
    {
        ...
        $sql = "SELECT id_beneficiaries, int_ano_mes_ref, str_uf, int_cod_siaf, str_nome_municipio, str_nis_favorecido, str_sit_benefic, int_valor_parcela, str_nome_favorecido FROM tb_beneficiaries_peti LIMIT {$linha_inicial}, " . QTDE_REGISTROS;
        ...
        $sqlContador = "SELECT COUNT(*) AS total_registros FROM tb_beneficiaries_peti";
...
//id_beneficiaries, int_ano_mes_ref, str_uf, int_cod_siaf, str_nome_municipio, str_nis_favorecido, str_sit_benefic, int_valor_parcela, str_nome_favorecido
     ...
     <thead>
       <tr style='text-transform: uppercase;' class='active'>
        ...
        <th style='text-align: center; font-weight: bolder;'>ANO/MES (AAAAMM)</th>
        <th style='text-align: center; font-weight: bolder;'>UF</th>
        <th style='text-align: center; font-weight: bolder;'>SIAF</th>
        <th style='text-align: center; font-weight: bolder;'>MUNICIPIO</th>
        <th style='text-align: center; font-weight: bolder;'>NIS</th>
        <th style='text-align: center; font-weight: bolder;'>SITUAÇAO</th>
     ...
     <tbody>";
        ...
        <td style='text-align: center'>$bene->str_uf</td>
        <td style='text-align: center'>$bene->int_cod_siaf</td>
        <td style='text-align: center'>$bene->str_nome_municipio</td>
        <td style='text-align: center'>$bene->str_nis_favorecido</td>
        <td style='text-align: center'>$bene->str_sit_benefic</td>
